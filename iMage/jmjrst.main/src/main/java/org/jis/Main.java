...
public class Main extends JFrame {
	...
	private void init() {
		...
		try {
			jOutputDoc.insertString(jOutputDoc.getLength(), mes.getString("Main.2") + Options.ls,
					readyAtr);
	...
	public static void main(String[] args) {
		if (args.length == 0)
			...
			for (int i = 0; i < args.length; i++) {
				...
				try {
					if (args[i].substring(0, 4).equalsIgnoreCase(mes.getString("hmax")))
						hmax = args[i].substring(args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 4).equalsIgnoreCase(mes.getString("vmax")))
						vmax = args[i].substring(args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 5).equalsIgnoreCase(mes.getString("input")))
						input = args[i].substring(args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 6).equalsIgnoreCase(mes.getString("output")))
						output = args[i].substring(args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 7).equalsIgnoreCase(mes.getString("quality")))
						quality = args[i].substring(args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
				} catch (Exception e) {
				}
			}
			...
			System.out.println(mes.getString("Main.30") + fi.toString() + mes.getString("Main.31")
					+ fo.toString() + mes.getString("Main.32") + q); //$NON-NLS-1$
